...
import {
  Users, Clock, CheckCircle, XCircle, Calendar, BarChart3, Settings, LogOut,
  Fingerprint, UserCheck, TrendingUp, Activity, DollarSign, Receipt
} from "lucide-react";
...
const Dashboard = () => {
  ...
  const recentActivities = attendanceRecords
    ...
    .map(record => {
      ...
      return {
        ...
        time: new Date(record.check_out || record.check_in).toLocaleTimeString(),
        status:
  ...
  const attendanceRate =
    employees.length > 0 ? ((presentCount / employees.length) * 100).toFixed(1) : "0.0";
